...
![image-20200618223914052](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20200618223914052.png)
...
其中的`SqlSessionFactory`,`SqlSession`是 MyBatis 接口的核心类，尤其是 SqlSession，这个接口是 MyBatis 中最重要的接口，这个接口能够让你**执行命令，获取映射，管理事务**。
...
SQL 语句的执行涉及多个组件，包括 MyBatis 的四大组件，它们是: `Executor`、`StatementHandler`、`ParameterHandler`、`ResultSetHandler`。SQL 的执行过程可以用下面这幅图来表示
...
Mybatis 中有`一级缓存`和`二级缓存`，这两级缓存都依赖于缓存模块中的实现。但是需要注意，**这两级缓存与 Mybatis 以及整个应用是运行在同一个 JVM 中的，共享同一块内存，如果这两级缓存中的数据量较大**，则可能影响系统中其它功能，所以需要**缓存大量数据时**，优先考虑使用 Redis、Memcache 等缓存产品。
...
#{}是预编译处理（一般用这个），${}是字符串替换。mybatis在处理#{}时，**会将sql中的#{}替换为?号**，调用PreparedStatement的set方法来赋值；mybatis在处理${}时，就是把${}替换成变量的值。**使用#{}可以有效的防止SQL注入，提高系统安全性**。 
